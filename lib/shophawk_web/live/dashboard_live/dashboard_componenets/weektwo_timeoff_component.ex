defmodule ShophawkWeb.WeektwoTimeoffComponent do
  ...
  def render(assigns) do
    ...
    <div class="text-center justify-center rounded p-4 bg-cyan-900 m-2 h-[43vh]">
        ...
        <%= if not Enum.empty?(@weekly_dates) do  %>
          Time Off Next Week <%= Calendar.strftime(@weekly_dates.next_monday, "%m-%d") %> to <%= Calendar.strftime(@weekly_dates.next_friday, "%m-%d") %>
        <% else %>
          Time Off Next Week
        <% end %>
        </div>
        ...
        <div class="text-xl bg-cyan-800 rounded m-2 p-2 h-[87%] overflow-y-auto sm:text-lg md:text-xl lg:text-2xl">
          <div class="grid grid-cols-5 text-center">
            <%= for {key, values} <- @week2_timeoff do %>
              <div>
